    ...
    <div className="bg-white border border-gray-200 rounded-xl mt-3 overflow-hidden">
        <div className="flex gap-4 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
            {Array.from({ length: cols }).map((_, j) => (
                <div key={j} className="h-2.5 bg-gray-100 rounded-full animate-pulse"
                    style={{ width: `${50 + (j * 11) % 40}px` }} />
            ))}
        </div>
...
const AutoInsurance = () => {
    ...
    const loading = !validatorDocDetails || Object.keys(validatorDocDetails).length === 0;
...
console.log("selectedData",selectedData)
    return (
        <div className="min-w-0 w-full">
            <ValidatorBreadcrumb
                crumbs={[{ label: "ValidatorSetup", screen: "validatorsetup" }, { label: selectedData.name }]}
            />
                        ...
                        >
                            {tabs.map((t) => (
                                <button
                                    ...
                                    className={`relative flex items-center gap-1.5 px-4 py-3 text-[13px] font-semibold whitespace-nowrap flex-shrink-0 transition-colors duration-200 ${
                                        activeTab === t.key
                                            ? "text-[#6B55E8]"
