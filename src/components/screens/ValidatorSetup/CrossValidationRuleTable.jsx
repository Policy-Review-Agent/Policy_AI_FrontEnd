...
import {
    createValidatorCrossCheckDetails,
    updateValidatorCrossCheckDetails,
    deleteValidatorCrossCheckDetails,
    getValidatorDocDetails,
    getPolicyExtractedFields
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/40 px-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4">
            ...
            <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <span className="font-semibold text-gray-800 font-mono">"{rule?.check_name || rule?.name}"</span>?
            </p>
...
// ── Custom Dropdown ───────────────────────────────────────────────────────────
// Accepts both flat string[] and { label, value }[] — normalises internally
        ...
        <div ref={ref} className="relative flex-1 min-w-0">
            <div
                ...
                ${disabled
                        ...
                        : "bg-white cursor-pointer hover:border-[#6B55E8]"
                    }`}
            >
            ...
            {open && !disabled && (
                <div
                    className={`absolute left-0 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-40 overflow-y-auto ${openUpward ? "bottom-full mb-1" : "top-full mt-1"
                        }`}
                >
                    {normalised.map((item) => (
                        <div
...
const ExtraMappingsBadge = ({ mappings, docNameToLabel }) => {
    ...
    const handleToggle = (e) => {
        ...
        if (!open && ref.current) {
            ...
            const style = {};
            if (spaceRight < 220) { style.right = 0; style.left = "auto"; }
            else { style.left = 0; style.right = "auto"; }
            if (spaceBelow < 200) { style.bottom = "100%"; style.top = "auto"; style.marginBottom = "4px"; }
            else { style.top = "100%"; style.bottom = "auto"; style.marginTop = "4px"; }
            setPopupStyle(style);
        }
        setOpen((o) => !o);
    };
                ...
                >
                    ...
                    <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                        {mappings.map((m, i) => {
                            ...
                            const docDisplay = docNameToLabel[rawDoc] || rawDoc;
                            return (
                                <div key={i} className="flex items-center gap-1.5 flex-wrap">
                                    <span className="text-[11px] font-semibold text-gray-700 bg-gray-50 border border-gray-200 px-2 py-0.5 rounded-md">
                                        {docDisplay}
                                    </span>
                                    <span className="text-gray-300 text-xs">→</span>
                                    <span className="text-[11px] font-medium text-indigo-500 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md font-mono">
                                        {m.field}
                                    </span>
                                </div>
                            );
...
const MappingsList = ({ rule }) => {
    ...
    const docNameToLabel = useMemo(() =>
        Object.fromEntries(
            (documentTypes?.data || []).map((d) => [d.name, d.display_name])
        ),
        [documentTypes]);
    ...
    return (
        <div className="flex flex-col gap-1.5">
            {visibleMappings.map((m, i) => {
                ...
                return (
                    <div key={i} className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-[11px] font-semibold text-gray-700 bg-gray-50 border border-gray-200 px-2 py-0.5 rounded-md whitespace-nowrap">
                            {docDisplay}
                        </span>
                        <span className="text-gray-300 text-xs">→</span>
            ...
            {hiddenMappings.length > 0 && (
                <ExtraMappingsBadge mappings={hiddenMappings} docNameToLabel={docNameToLabel} />
            )}
        </div>
    );
};
...
const MappingRow = ({ mapping, index, onChange, onRemove }) => {
    ...
    const { documentTypes, policyExtractedFields } = useSelector(
        (state) => state.validatorSetup
    );
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl px-2 py-3">
            <div className="grid grid-cols-[1fr_auto_1fr_auto] items-center mb-2">
                ...
                <button
                    ...
                    className="p-1 border border-red-200 text-red-400 rounded"
                >
                    <X size={10} />
                </button>
            </div>
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-1 items-center">
                ...
                <CustomDropdown
                    ...
                    onChange={(val) => {
                        ...
                        onChange(index, "isDocChanged", true); // ✅ persist
...
const RulePanel = ({ open, onClose, onSave, editRule, loading }) => {
    ...
    const FIELD_OPTIONS = policyExtractedFields || []; // Debug log to verify extracted fields are in state
                ...
                setMappings(
                    existing.length
                        ? existing.map((m, i) => ({
                            ...
                            field: m.field,
                            isDocChanged: i === 0 // ✅ first row enabled
                        }))
                        : [{
                            document_type: defaultDocValue,
                            field: "",
                            isDocChanged: true
                        }]
                );
    ...
    const addMapping = () =>
        setMappings((p) => [
            ...p,
            { document_type: "", field: "", isDocChanged: false }
        ]);
                ...
                <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        <h2 className="text-[17px] font-bold text-gray-800">
                            {isEdit ? "Edit Validation Rule" : "Add Validation Rule"}
                        </h2>
                        <p className="text-[12px] text-gray-400 mt-0.5">Define a cross-document field consistency check.</p>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-1.5 rounded-lg border border-gray-200 text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors flex-shrink-0"
                    >
                        <X size={14} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
                        ...
                        {matchOpen && (
                            <div className="absolute left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                                {MATCH_TYPE_OPTIONS.map((o) => (
                                    <div
                                        key={o.value}
                                        onClick={() => { setMatchType(o.value); setMatchOpen(false); }}
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-gray-400 mb-3">
                            Map a field from each document that should match under this rule.
                        </p>
                        <div className="flex flex-col gap-2">
                            {mappings.map((m, i) => (
                                // Debug log to check mapping data passed to MappingRow
                                <MappingRow
                                    key={i}
                                    mapping={m}
                                    index={i}
...
const CrossValidationRuleTable = () => {
    ...
    const handleSaveRule = async ({ id, payload }) => {
        ...
        try {
            if (id) {
                ...
                if (error || !data) {
                    addToast("error", "Update Failed", error || "Could not update the rule.");
                } else {
                    await refreshRules();
                    addToast("success", "Rule Updated", `"${payload.check_name}" updated successfully.`);
                    closePanel();
                }
            } else {
                const { error } = await createValidatorCrossCheckDetails(selectedPolicyId, payload);
                if (error) {
                    addToast("error", "Create Failed", "Could not create the rule.");
                } else {
                    await refreshRules();
                    addToast("success", "Rule Added", `"${payload.check_name}" added successfully.`);
                    closePanel();
                }
            }
        } finally {
    ...
    const handleDeleteConfirm = async () => {
        ...
        if (error) {
            addToast("error", "Delete Failed", "Could not remove the rule.");
        } else {
            await refreshRules();
            addToast("error", "Rule Removed", `"${name}" has been removed.`);
        }
    };
    ...
    const StatusBadge = ({ isActive }) => (
        <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full border ${isActive
            ...
            <div className="flex items-start justify-between flex-wrap gap-4">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-1.5">
                        <span className="bg-gray-100 text-gray-500 border border-gray-200 text-[11px] font-semibold px-3 py-0.5 rounded-full">
                            {rules.length} Total
                        </span>
                        <span className="bg-green-50 text-green-600 border border-green-200 text-[11px] font-semibold px-3 py-0.5 rounded-full">
                            {active} Active
                        </span>
                        <span className="bg-gray-100 text-gray-500 border border-gray-200 text-[11px] font-semibold px-3 py-0.5 rounded-full">
                            {inactive} Inactive
                    ...
                    <button
                        onClick={openAdd}
                        className="flex items-center gap-1 text-[12px] font-semibold text-white bg-[#6B55E8] hover:bg-[#5a45d4] px-3 py-1.5 rounded-md transition-all"
                    >
                        + Add Rule
                    </button>
                </div>
                            ...
                            {rules.map((rule) => (
                                <tr key={rule.id} className="hover:bg-gray-50 transition-colors align-top">
                                    <td className="px-5 py-4 w-[180px]">
                                        <span className="text-[12px] font-bold text-gray-800 font-mono break-all">
                                            {rule.check_name || rule.name}
                                        </span>
                                    </td>
                                    <td className="px-5 py-4 w-[200px]">
                                        <p className="text-[12px] text-gray-400 font-medium leading-relaxed">{rule.description}</p>
                                    </td>
                                    ...
                                    <td className="px-5 py-4 w-[100px]">
                                        <div className="flex items-center gap-2">
                                            <button
                                                onClick={() => {
                                                    openEdit(rule)
                                                    getPolicyExtractedFields(rule.field_mappings[0]?.document_type, dispatch, setPolicyExtractedFields);
                                                }}
                                                className="p-1.5 rounded-md border border-gray-200 text-gray-400 hover:text-indigo-500 hover:border-indigo-200 transition-colors"
                                                title="Edit"
                                            >
                                                <Pencil size={12} />
                                            </button>
            ...
            <div className="md:hidden flex flex-col gap-2">
                {rules.map((rule) => (
                    <div key={rule.id} className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-2">
                            <span className="text-[12px] font-bold text-gray-800 font-mono break-all flex-1">
                                {rule.check_name || rule.name}
                            </span>
                ...
                {rules.length === 0 && (
                    <p className="text-center text-[13px] text-gray-400 py-10">
                        No rules yet. Tap <strong>+ Add Rule</strong> to create one.
                    </p>
                )}
            </div>
