...
//             {/* ── Logo + collapse arrow ── */}
//             <div className="flex items-center gap-2.5 px-5 py-4 border-b-[1.5px] border-gray-100">
//                 <div className="w-8 h-8 rounded-lg bg-color bg-gradient-to-br from-primary to-purple-500 flex items-center justify-center shadow-md shadow-primary/30 flex-shrink-0">
//                     <Shield size={14} className="text-white" />
//                 </div>
//                 <span className="text-[15px] font-bold text-gray-800 flex-1">
...
//                     <div className="w-7 h-7 rounded-full bg-color bg-gradient-to-br from-primary to-purple-500 flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0">
//                         SR
//                     </div>
//                     <div>
...
const DEFAULT_BATCH_ID = "BATCH-2024-031";
...
const Sidebar = ({ onClose, onNavigate }) => {
    ...
    const handleNav = (id) => {
        // ✅ Always reset to first batch + first policy before navigating
        dispatch(selectBatch(DEFAULT_BATCH_ID)); // reset batch to first
        dispatch(selectPolicy(0));               // reset policy index to first
        dispatch(navigate(id));
        onNavigate?.();
    };
            ...
            <nav className="flex-1 px-3 py-4">
                ...
                {NAV_ITEMS.map(({ id, label, Icon }) => (
                    <button
                        key={id}
                        onClick={() => handleNav(id)}
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] font-medium mb-0.5 transition-all ${
                            screen === id
                                ? "bg-blue-50 text-blue-600 font-semibold"
                                : "text-gray-500 hover:bg-gray-50 hover:text-gray-800"
                        }`}
